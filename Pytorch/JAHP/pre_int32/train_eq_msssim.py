...
from JAHPmodel_eq import JointAutoregressiveHierarchicalPriors, load_checkpoint_int32_actft, save_checkpoint_int32_actft
...
def act_t_replace(container, target_layer_list, prefix=''):
    global act_Bits, dequantize, device
    for name, module in container.named_children():
        full_name = prefix + name
...
def w_scale(model):
    ...
    #####eq
    for k in model_dict:
        if "g_s" in k and "weight" in k:
            ord = k.replace("module.", "").split('.')[1][-1]
            if model.eqlayer == int(ord):
                next_w = k.replace(str(model.eqlayer), str(model.eqlayer + 2))
                avrg_s = model_dict["s"] / model_dict["sn"]
                model_dict[k] *= avrg_s.reshape(1, -1, 1, 1)
                model_dict[k.replace("weight", "bias")] *= avrg_s.reshape(-1)
                model_dict[next_w] = model_dict[next_w] / avrg_s
...
def main():
    global use_cuda, act_target_layer, muls_m, w_Bits, act_Bits, dequantize, device
    epochs = 10
    weight_decay = 0.1
    clip_max_norm = 1.0
    data_dir = "/data0/fym"
    ...
    muls_m = {"h_s":[25, 25, 25], "h_a":[22, 23, 25], 
                "g_s":[23, 23, 24, 28], "g_a":[16, 25, 24, 23], #####[17, 25, 25, 23]?
    ...
    act_Bits = 8 #####
    train_lambda = 115.37 #####
    loadFP = True ##### False -- preiter True -- "checkpoint_best_loss.pth.tar"
    preiter = 2 #####
    lr = 1e-6 ##### 1e-5 1e-6 1e-7 (Adam)
    aux_lr = lr * 0.1
    eqlayer = 0 ##### 0, 2, 4
    eqrange = 3 #####
    optim_Adam = True #####
    save_path = "pre_int32/checkpoints/eq_msssim_" + str(train_lambda) + "/" #####
    if not os.path.isdir(save_path):
        os.mkdir(save_path)
    dir = "FP" if loadFP else "pre_int32"
    if loadFP:
        load_path = os.path.join(dir, "checkpoints/msssim_" + str(train_lambda))
        model_name = "checkpoint_best_loss.pth.tar"
    else:
        load_path = os.path.join(dir, "checkpoints/eq_msssim_" + str(train_lambda))
        model_name = "iter_" + str(preiter) + "/iter_" + str(preiter) + ".pth.tar"
    ...
    if train_lambda < 31.73:
        ...
        N, M = 192, 320
    net = JointAutoregressiveHierarchicalPriors(eqlayer, eqrange, N, M)
    net = net.to(device)
    ...
    if loadFP:  # load from previous checkpoint
        ...
        best_loss = checkpoint["loss"] * 1.5
        checkpoint["state_dict"]["s"] = net.state_dict()["s"]
        checkpoint["state_dict"]["sn"] = net.state_dict()["sn"]
    ...
    for k, v in state_dict.items():
        state_dict[k] = Variable(v.float().to(device), requires_grad=True)
    ...
    global model_dir
    model_dir = "pre_int32/checkpoints/eq_msssim_" + str(train_lambda)
    if not os.path.isdir(model_dir):
        os.mkdir(model_dir)
    f = open(model_dir + "/log.txt", "a")
    f.write("\n\nbegin to log the best model with Loss:{:.6f} and lr:{:.2e}".format(best_loss, optimizer.param_groups[0]['lr']))
    f.write("\nmuls_m: {}".format(muls_m))
    f.write("\nequalized layer: {} and equalized range: {}".format(eqlayer, eqrange))
    f.close()
