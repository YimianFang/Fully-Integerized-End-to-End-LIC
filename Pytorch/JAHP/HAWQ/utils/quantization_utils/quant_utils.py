...
def batch_frexp_int32(inputs, set_e):
    """
    Decompose the scaling factor into mantissa and twos exponent.
    ...
    k=0
    for m in output_m:
        m_s = output_e[k] + set_e
        int_m_shifted = int(Decimal(m * (2 ** m_s)).quantize(Decimal('1'), rounding=decimal.ROUND_HALF_UP))
        tmp_m.append(int_m_shifted)
        tmp_e.append(set_e)
        k += 1
    output_m = np.array(tmp_m)
    output_e = np.array(tmp_e)
    ...
    def forward(ctx, z, bitwidth, quant_mode, z_scaling_factor, case, pre_act_scaling_factor=None,
        ...
        with torch.no_grad():
            ...
            if case == 0:
                ...
                # c = z_int.shape[1]
                # # set_e = 31 - np.frexp(z_int.permute(1, 0, 2, 3).reshape(c, -1).max(-1)[0].cpu().numpy())[1].max() #####
                # set_e = 23 #####
                # m, e = batch_frexp_int32(new_scale, set_e)

                m, e = batch_frexp(new_scale)

                output = z_int.type(torch.double) * m.type(torch.double)
                # if (torch.log2(output.abs().max()) > 31):
                #     print("-------------")
                #     print("Overflow!!!")
                #     print("-------------")
