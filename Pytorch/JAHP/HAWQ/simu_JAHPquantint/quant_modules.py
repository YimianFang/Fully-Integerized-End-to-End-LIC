...
class QuantAct(Module):
    ...
    def __init__(self,
        ...
        self.register_buffer('x_min', torch.zeros([]))
        self.register_buffer('x_max', torch.zeros([]))
        self.register_buffer('act_scaling_factor', torch.zeros([]))

        self.register_buffer('pre_weight_scaling_factor', torch.ones([]))
        self.register_buffer('identity_weight_scaling_factor', torch.ones(1))
...
class QuantConv2d(Module):
    ...
    def set_param_TransConv(self, conv):
        self.in_channels = conv.in_channels
        self.out_channels = conv.out_channels
        self.kernel_size = conv.kernel_size
        self.stride = 1
        ...
        self.register_buffer('conv_scaling_factor', torch.zeros(self.out_channels))
        self.weight = Parameter(torch.flip(conv.weight.data.clone().permute(1, 0, 2, 3), [2,3]))
    ...
    def forward(self, x, pre_act_scaling_factor=None):
        ...
        if self.bias is None:
            ...
            return (F.conv2d(x_int, self.weight_integer, self.bias_integer, self.conv.stride, self.conv.padding,
                             self.conv.dilation, self.conv.groups), self.conv_scaling_factor)
