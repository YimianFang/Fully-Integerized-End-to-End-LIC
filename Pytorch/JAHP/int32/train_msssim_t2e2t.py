...
from JAHPmodel_msssim import JointAutoregressiveHierarchicalPriors, load_qftmodel, load_t2e2tmodel, save_model
import os
...
def parse_args(argv):
    ...
    parser.add_argument("--pretrain", action="store_true", default=False, help="load a pretrain pre_int32 model")
...
def main(argv):
    ...
    if args.lmbda < 31.73:
        ...
        N, M = 192, 320
    if args.eq:
        prepath = "pre_int32/checkpoints/eq_msssim_" + str(args.lmbda) #####
    else:
        prepath = "pre_int32/checkpoints/msssim_" + str(args.lmbda) #####
    ...
    best_loss = float("inf")
    last_epoch = 1
    if args.checkpoint and not args.pretrain:  # load from previous checkpoint
        print("Loading checkpoint", args.checkpoint)
        global_step, best_loss = load_t2e2tmodel(net, args.checkpoint)
    elif args.pretrain and not args.checkpoint:
        pretrain_path = os.path.join(prepath, f"iter_{args.preiter}", f"iter_{args.preiter}.pth.tar")
        print("Loading pretrained model", pretrain_path)
        global_step = load_qftmodel(net, pretrain_path)
    else:
        raise Exception("Wrong format to load models!")

    if args.eq:
        model_dir = "int32/checkpoints/eq_msssim_" + str(args.lmbda)
    else:
        model_dir = "int32/checkpoints/msssim_" + str(args.lmbda)
