...
def get_sf_CW(model, state_dict, overflow_rate=0):
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d):
            ...
            para = state_dict[(name + '.weight').replace('module.','')].permute(1, 0, 2, 3).flip([2,3])
            abs_value = para.abs().reshape(para.shape[0], -1)
            sf = []
            for i in range(para.shape[0]):
                sorted_value = abs_value[i].sort(dim=0, descending=True)[0]
                split_idx = int(overflow_rate * (len(sorted_value) / 2))
...
def get_sf_CW_prior(model, state_dict, overflow_rate=0):
    sf_list = []
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d) and "prior" in name:
            para = state_dict[(name + '.weight').replace('module.','')]
            abs_value = para.abs().view(para.shape[0], -1)
            sf = []
            for i in range(para.shape[0]):
...
def changemodelbit_Waseda_CW(Bits,model,sf_list,state):
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d):
            ...
            v = state[(name + '.weight').replace('module.','')].permute(1, 0, 2, 3).flip([2,3])
            if isinstance(Bits, int):
                bits = Bits
            else:
                bits = Bits[kk]
            v_quant = Waseda_quantize_CW(v, sf_list[kk], bits=bits)
            kk = kk + 1
            state_dict_quant[name + '.weight'] = v_quant.permute(1, 0, 2, 3).flip([2,3])
...
def changemodelbit_CW_prior(Bits,model,sf_list,state):
    state_dict_quant = model.state_dict()
    kk = 0
    for name, module in model.named_modules():
        if isinstance(module, nn.Conv2d) and "prior" in name:
