...
from model_stat import JointAutoregressiveHierarchicalPriors
import os
...
def test_epoch(epoch, test_dataloader, model, criterion):
    ...
    mem["h_s"] = [float("-inf")] * 5
    mem["g_s"] = [float("-inf")] * 7
    mem["entropy"] = [float("-inf")] * 5
    mem["context"] = float("-inf")
    ...
    return loss.avg, psnr.avg, bpp_loss.avg, msssimDB.avg, mem
...
def main(argv):
    ...
    f.close()
    if args.checkpoint:
        _, _, _, _, mem = test_epoch(last_epoch - 1, test_dataloader, net, criterion)
        print(mem)
