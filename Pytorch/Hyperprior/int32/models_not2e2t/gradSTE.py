...
def t2e_p(mtx):
    return (torch.round(2 ** (torch.log2(mtx+1) * 7 / 11))-1).clamp_(-128, 127)

def e2t_p(mtx):
    return torch.round(2 ** (torch.log2(mtx+1) * 11 / 7))-1

def t2e_n(mtx):
    return (-torch.round(2 ** (torch.log2(-mtx) * 7 / 11))).clamp_(-128, 127)

def e2t_n(mtx):
    return -torch.round(2 ** (torch.log2(-mtx) * 11 / 7))
