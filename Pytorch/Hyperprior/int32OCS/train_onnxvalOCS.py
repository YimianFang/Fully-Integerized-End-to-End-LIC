...
def testKodak(step, if_prnt=True):
    with torch.no_grad():
        ...
        for batch_idx, input in enumerate(test_loader):
            ##### test for ppm 256*256 patch (.pth.tar) #####
            input = torch.load("imout.pth.tar")
            ##### test for ppm 256*256 patch (.jpg) #####
            # input = torch.clip(torch.round(input * (2 ** 8)), 0, 255)
            # torch.save(input, "ppm/jpgval/kodim08_patch_input.pth.tar")
            #####
            clipped_recon_image, mse_loss, bpp_feature, bpp_z, bpp = net(input)
...
if __name__ == "__main__":
    ...
    if args.pretrain != '':
        ...
        global_step = load_t2e2tmodel_onnxval(model, args.pretrain) #####
    net = model.cuda() ###cuda
    # net = model.cpu() ###cpu
