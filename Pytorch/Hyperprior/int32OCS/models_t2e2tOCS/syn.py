...
class Synthesis_net(nn.Module):
    ...
    def forward(self, x, muls, relus, Bits=8, split=4):
        ...
        # x = self.deconv1(x)
        x = floorSTE.apply((x + (2 ** 3)) / (2 ** 4)) #####针对ana最后一层的改进，少除2**4
        xout = 0.
        for i in range(split):
            xout += F.conv_transpose2d(x, 
                        floorSTE.apply((wSTE.apply(self.deconv1.weight) + i) / split), 
                        floorSTE.apply((roundSTE.apply(self.deconv1.bias) + i) / split), 
                        self.deconv1.stride, self.deconv1.padding, self.deconv1.output_padding)
        # x = x.permute(1, 0, 2, 3)
        ...
        for i in range(split):
            xout += F.conv_transpose2d(x, 
                        floorSTE.apply((wSTE.apply(self.deconv2.weight) + i) / split), 
                        floorSTE.apply((roundSTE.apply(self.deconv2.bias) + i) / split), 
                        self.deconv2.stride, self.deconv2.padding, self.deconv2.output_padding)
        # x = x.permute(1, 0, 2, 3)
        ...
        for i in range(split):
            xout += F.conv_transpose2d(x, 
                        floorSTE.apply((wSTE.apply(self.deconv3.weight) + i) / split), 
                        floorSTE.apply((roundSTE.apply(self.deconv3.bias) + i) / split), 
                        self.deconv3.stride, self.deconv3.padding, self.deconv3.output_padding)
        # x = x.permute(1, 0, 2, 3)
        ...
        for i in range(split):
            xout += F.conv_transpose2d(x, 
                        floorSTE.apply((wSTE.apply(self.deconv4.weight) + i) / split), 
                        floorSTE.apply((roundSTE.apply(self.deconv4.bias) + i) / split), 
                        self.deconv4.stride, self.deconv4.padding, self.deconv4.output_padding)
        # x = x.permute(1, 0, 2, 3)
