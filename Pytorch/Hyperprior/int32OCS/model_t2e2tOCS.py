...
class ImageCompressor(nn.Module):
    ...
    def forward(self, input_image):
        ...
        mulsE = torch.load("/data1/fym/pre_int32/checkpoints/qftssim_128/iter_324354_muls_Encoder.pth.tar", map_location=device)
        relusE = torch.load("/data1/fym/pre_int32/checkpoints/qftssim_128/iter_324354_relus_Encoder.pth.tar", map_location=device)
        ...
        mulspE = torch.load("/data1/fym/pre_int32/checkpoints/qftssim_128/iter_324354_muls_priorEncoder.pth.tar", map_location=device)
        reluspE = torch.load("/data1/fym/pre_int32/checkpoints/qftssim_128/iter_324354_relus_priorEncoder.pth.tar", map_location=device)
        ...
        mulspD = torch.load("/data1/fym/pre_int32/checkpoints/qftssim_128/iter_324354_muls_priorDecoder.pth.tar", map_location=device)
        reluspD = torch.load("/data1/fym/pre_int32/checkpoints/qftssim_128/iter_324354_relus_priorDecoder.pth.tar", map_location=device)
        ...
        mulsD = torch.load("/data1/fym/pre_int32/checkpoints/qftssim_128/iter_324354_muls_Decoder.pth.tar", map_location=device)
        relusD = torch.load("/data1/fym/pre_int32/checkpoints/qftssim_128/iter_324354_relus_Decoder.pth.tar", map_location=device)
        ...
        # compressed_feature_renorm_1 = torch.floor((compressed_feature_renorm + 2 ** 3) / 2 ** 4)###针对ana最后一层的改进，少除2**4  #####
        compressed_feature_renorm_2 = torch.round(compressed_feature_renorm / 2 ** 4)###针对ana最后一层的改进，少除2**4  #####
        # torch.save(compressed_feature_renorm, "compression/y/y"+str(self.n)+"_code.pth.tar")
        # self.n += 1
        total_bits_feature, _ = feature_probs_based_sigma(compressed_feature_renorm_2, recon_sigma)
