...
struct SampleINT8APIPreprocessing
{
    ...
    std::vector<int> inputDims{1,3,256,256};
};
...
private:
    ...
    bool WriteResults(const samplesCommon::BufferManager& buffers);
...
bool SampleINT8API::WriteResults(const samplesCommon::BufferManager& buffers)
{
    // const float* x = static_cast<const float*>(buffers.getHostBuffer("input"));
    // std::cout << buffers.size("input") << std::endl;
    // const float* E_1 = static_cast<const float*>(buffers.getHostBuffer("E_1"));
    // std::cout << buffers.size("E_1") << std::endl;
    // const float* E_2 = static_cast<const float*>(buffers.getHostBuffer("E_2"));
    // std::cout << buffers.size("E_2") << std::endl;
    // const float* E_3 = static_cast<const float*>(buffers.getHostBuffer("E_3"));
    // std::cout << buffers.size("E_3") << std::endl;
    const float* y = static_cast<const float*>(buffers.getHostBuffer("y"));
    std::cout << buffers.size("y") << std::endl;
    const float* z = static_cast<const float*>(buffers.getHostBuffer("z"));
    std::cout << buffers.size("z") << std::endl;
    // const float* x = static_cast<const float*>(buffers.getHostBuffer("x"));
    // std::cout << buffers.size("x") << std::endl;
    // const float* sigma = static_cast<const float*>(buffers.getHostBuffer("recon_sigma"));
    // std::cout << buffers.size("recon_sigma") << std::endl;
    // const float* x_hat = static_cast<const float*>(buffers.getHostBuffer("recon_image"));
    // std::cout << buffers.size("recon_image") << std::endl;

    std::ofstream outfile;
    // outfile.open("x.txt", std::ios::app);
    // if(!outfile.is_open ())
    //     std::cout << "Open file failure" << std::endl;
    // for (int i = 0, volImg = 3 * 256 * 256; i < volImg; ++i)
    // {
    ...
    // outfile.open("E_1.txt", std::ios::app);
    // if(!outfile.is_open ())
    //     std::cout << "Open file failure" << std::endl;
    // for (int i = 0, volImg = 128 * 128 * 128; i < volImg; ++i)
    // {
    //     outfile << E_1[i] << std::endl;
    // }
    // outfile.close();
    // std::cout << "E_1" << std::endl;

    // outfile.open("E_2.txt", std::ios::app);
    // if(!outfile.is_open ())
    //     std::cout << "Open file failure" << std::endl;
    // for (int i = 0, volImg = 128 * 64 * 64; i < volImg; ++i)
    // {
    //     outfile << E_2[i] << std::endl;
    // }
    // outfile.close();
    // std::cout << "E_2" << std::endl;

    // outfile.open("E_3.txt", std::ios::app);
    // if(!outfile.is_open ())
    //     std::cout << "Open file failure" << std::endl;
    // for (int i = 0, volImg = 128 * 32 * 32; i < volImg; ++i)
    // {
    //     outfile << E_3[i] << std::endl;
    // }
    // outfile.close();
    // std::cout << "E_3" << std::endl;

    outfile.open("y.txt", std::ios::app);
    if(!outfile.is_open ())
        std::cout << "Open file failure" << std::endl;
    for (int i = 0, volImg = 192 * 16 * 16; i < volImg; ++i)
    {
    ...
    outfile.open("z.txt", std::ios::app);
    if(!outfile.is_open ())
        std::cout << "Open file failure" << std::endl;
    for (int i = 0, volImg = 128 * 4 * 4; i < volImg; ++i)
    {
        outfile << z[i] << std::endl;
    }
    outfile.close();
    std::cout << "z" << std::endl;
    ...
    // outfile.open("sigma.txt", std::ios::app);
    // if(!outfile.is_open ())
    //     std::cout << "Open file failure" << std::endl;
    // for (int i = 0, volImg = 192 * 16 * 16; i < volImg; ++i)
    // {
    //     outfile << sigma[i] << std::endl;
    // }
    // outfile.close();
    // std::cout << "sigma" << std::endl;

    // outfile.open("x_hat.txt", std::ios::app);
    // if(!outfile.is_open ())
    //     std::cout << "Open file failure" << std::endl;
    // for (int i = 0, volImg = 3 * 256 * 256; i < volImg; ++i)
    // {
    //     outfile << x_hat[i] << std::endl;
    // }
    // outfile.close();
...
bool SampleINT8API::readPerTensorDynamicRangeValues()
{
    ...
    while (std::getline(iDynamicRangeStream, line))
    {
        ...
        // std::cout << dynamicRange << std::endl;
...
void SampleINT8API::setLayerPrecision(SampleUniquePtr<nvinfer1::INetworkDefinition>& network)
{
    ...
    for (int i = 0; i < network->getNbLayers(); ++i)
    {
        ...
        if (layer->getType() == LayerType::kCONVOLUTION
                || layer->getType() == LayerType::kDECONVOLUTION)
        {
...
bool SampleINT8API::setDynamicRange(SampleUniquePtr<nvinfer1::INetworkDefinition>& network)
{
    ...
    sample::gLogInfo << "Setting Per Tensor Dynamic Range for Inputs" << std::endl;
    ...
    sample::gLogInfo << "Setting Per Tensor Dynamic Range for Layer Outputs" << std::endl;
    ...
    for (int i = 0; i < network->getNbLayers(); ++i)
    {
        ...
        for (int j = 0, e = lyr->getNbOutputs(); j < e; ++j)
        {
            ...
            int nPos = tName.find_last_of(":");
            // std::cout << nPos << std::endl;
            if(nPos != -1) 
            {
                tName = tName.substr(nPos + 1, tName.length() - nPos + 1);
            }
            // std::cout << tName << std::endl;
            if (mPerTensorDynamicRangeMap.find(tName) != mPerTensorDynamicRangeMap.end())
            {
    ...
    for (int c = 0; c < channels; ++c)
    {
        for (int h = 0; h < height; ++h)
        {
            for (int w = 0; w < width; ++w)
            {
                ...
                hostInputBuffer[dstIdx] = static_cast<const float>(value);
            }
        }
    }
    return true;
...
bool SampleINT8API::serialize()
{
    ...
    std::cout << "Saving trt file." << std::endl;

    IHostMemory* serialized_model (mEngine->serialize());
    std::ofstream f ("int8LIC_E2.trt", std::ios::binary);
...
sample::Logger::TestResult SampleINT8API::infer()
{
    ...
    const float total = std::chrono::duration<float, std::milli>(t_end - t_start).count();
    sample::gLogInfo << "Elapse time is " << total << " ms." << std::endl;
...
struct SampleINT8APIArgs : public samplesCommon::Args
{
    bool verbose{false};
    bool writeNetworkTensors{false};
    std::string modelFileName{"int8LIC_E2.onnx"};
    std::string imageFileName{"kodim08_patch.txt"};
    std::string referenceFileName{"reference_labels.txt"};
    std::string dynamicRangeFileName{"int8LIC_E_per_tensor_dynamic_range.txt"};
    std::string networkTensorsFileName{"int8LIC_E_tensors.txt"};
};
...
SampleINT8APIParams initializeSampleParams(SampleINT8APIArgs args)
{
    ...
    if (args.dataDirs.empty()) //!< Use default directories if user hasn't provided directory paths
    {
        ...
        params.dataDirs.push_back("./data_resnet50");
    }
