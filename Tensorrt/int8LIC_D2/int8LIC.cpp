...
struct SampleINT8APIPreprocessing
{
    ...
    std::vector<int> inputDims{1,192,16,16};
};
...
bool SampleINT8API::WriteResults(const samplesCommon::BufferManager& buffers)
{
    ...
    // const float* y_1 = static_cast<const float*>(buffers.getHostBuffer("output_1"));
    // std::cout << buffers.size("output_1") << std::endl;
    // const float* y_2 = static_cast<const float*>(buffers.getHostBuffer("output_2"));
    // std::cout << buffers.size("output_2") << std::endl;
    const float* xhat = static_cast<const float*>(buffers.getHostBuffer("x"));
    ...
    // const float* z = static_cast<const float*>(buffers.getHostBuffer("compressed_z"));
    // std::cout << buffers.size("compressed_z") << std::endl;
    ...
    // outfile.open("output_1.txt", std::ios::app);
    // if(!outfile.is_open ())
    //     std::cout << "Open file failure" << std::endl;
    // for (int i = 0, volImg = 128 * 128 * 128; i < volImg; ++i)
    // {
    //     outfile << y_1[i] << std::endl;
    // }
    // outfile.close();
    // std::cout << "output_1" << std::endl;

    // outfile.open("output_2.txt", std::ios::app);
    // if(!outfile.is_open ())
    //     std::cout << "Open file failure" << std::endl;
    // for (int i = 0, volImg = 128 * 64 * 64; i < volImg; ++i)
    // {
    //     outfile << y_2[i] << std::endl;
    // }
    // outfile.close();
    // std::cout << "output_2" << std::endl;

    outfile.open("xhat.txt", std::ios::app);
    if(!outfile.is_open ())
        std::cout << "Open file failure" << std::endl;
    for (int i = 0, volImg = 3 * 256 * 256; i < volImg; ++i)
    {
        outfile << xhat[i] << std::endl;
    }
    outfile.close();
    ...
    // if(!outfile.is_open ())
    //     std::cout << "Open file failure" << std::endl;
    // for (int i = 0, volImg = 128 * 8 * 8; i < volImg; ++i)
    // {
    //     outfile << z[i] << std::endl;
    // }
    // outfile.close();
...
bool SampleINT8API::serialize()
{
    ...
    std::ofstream f ("int8LIC_D2.trt", std::ios::binary);
...
struct SampleINT8APIArgs : public samplesCommon::Args
{
    bool verbose{false};
    bool writeNetworkTensors{false};
    std::string modelFileName{"int8LIC_D2.onnx"};
    std::string imageFileName{"y.txt"};
    std::string referenceFileName{"reference_labels.txt"};
    std::string dynamicRangeFileName{"int8LIC_D_per_tensor_dynamic_range.txt"};
    std::string networkTensorsFileName{"int8LIC_D_tensors.txt"};
};
