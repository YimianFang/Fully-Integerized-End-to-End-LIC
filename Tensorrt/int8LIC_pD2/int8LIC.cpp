...
struct SampleINT8APIPreprocessing
{
    ...
    std::vector<int> inputDims{1,128,4,4};
};
...
bool SampleINT8API::WriteResults(const samplesCommon::BufferManager& buffers)
{
    ...
    const float* sigma = static_cast<const float*>(buffers.getHostBuffer("sigma"));
    std::cout << buffers.size("sigma") << std::endl;
...
bool SampleINT8API::serialize()
{
    ...
    std::ofstream f ("int8LIC_pD2.trt", std::ios::binary);
...
struct SampleINT8APIArgs : public samplesCommon::Args
{
    bool verbose{false};
    bool writeNetworkTensors{false};
    std::string modelFileName{"int8LIC_pD2.onnx"};
    std::string imageFileName{"z.txt"};
    std::string referenceFileName{"reference_labels.txt"};
    std::string dynamicRangeFileName{"int8LIC_pD_per_tensor_dynamic_range.txt"};
    std::string networkTensorsFileName{"int8LIC_pD_tensors.txt"};
};
