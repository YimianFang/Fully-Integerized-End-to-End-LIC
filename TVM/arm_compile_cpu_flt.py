...
relus = np.load("npy/relus_E.npy", allow_pickle=True).tolist()
fx = tvm.relay.Constant(tvm.nd.array(np.array([2 ** (16 + clp_k - 4)], dtype="float32")))
fx_s = tvm.relay.Constant(tvm.nd.array(np.array([16 + clp_k - 4], dtype="float32")))
...
fx = tvm.relay.Constant(tvm.nd.array(np.array([2 ** (16 + clp_k - 4)], dtype="float32")))
fx_s = tvm.relay.Constant(tvm.nd.array(np.array([16 + clp_k - 4], dtype="float32")))
fx_D1 = tvm.relay.Constant(tvm.nd.array(np.array([2 ** (16 + clp_k - 2)], dtype="float32")))
fx_s_D1 = tvm.relay.Constant(tvm.nd.array(np.array([16 + clp_k - 2], dtype="float32")))
d2 = tvm.relay.Constant(tvm.nd.array(np.array([2], dtype="float32")))
...
syn = relay.transpose(syn, [1, 0, 2, 3])
syn = relay.clip(syn, 0, clp0)
syn = relay.floor_divide(syn * scl0 + fx_D1 / d2, fx_D1)
...
lib.export_library("arm_cpu_t2e2tm_1080_flt.so")
